...
class ques
{
	...
	void solve() throws Exception
	{
		...
		while(t-->0) {
			...
			HashMap<Integer,Integer> getPos = new HashMap<>();
			
			for(int i=0;i<n;i++) {
				map.put(arr[i],jump[i]);
			}
			for(int i=0;i<n;i++) {
				getPos.put(arr[i],i);
			}
			...
			for(int i=1;i<n;i++) {
				int pos = getPos.get(brr[i]);
				int prev = getPos.get(brr[i-1]);
	            //out.println(pos+" "+prev);
				while(pos<=prev) {
					pos +=map.get(brr[i]);
					count++;
				}
				
				getPos.put(brr[i],pos);
				//out.println(getPos);
			}
